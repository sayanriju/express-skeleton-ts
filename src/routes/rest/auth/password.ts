...
import { slug as cuidSlug } from 'cuid';
...
export async function startWorkflow(req: Request, res: Response) {
  try {
    ...
    await user.save();
    try {
      const mailOpts: MailOptsInterface = {
        to: user.email,
      ...
      await sendMail('forgot-password', mailOpts);
    } catch (mailErr) {
...
export async function resetPassword(req: Request, res: Response) {
  try {
    const {
      ...
      password,
      email,
    }: { token: string; password: string; email: string } = req.body;
